...
例如项目中使用了 `A@1.0.0` 而 `B` 模块依赖了 `@A: '*'`，此时 `A` 安装了新版本可能会导致一些破坏性变更，我们想让 `B` 依赖用更低版本，就可以用该字段，注意，`npm@v7` 以下的版本无效，v7 虽然能安装低版本，但也没按指定配置安装，只是综合实际情况用了相近版本，yarn、pnpm 会按指定版本安装。
